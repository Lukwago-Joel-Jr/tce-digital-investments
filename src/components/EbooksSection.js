      ...
      <div className="flex flex-col md:flex-row gap-5 md:gap-30">
        {ebooks.map((book) => (
          <Link
            key={book.id}
            href={`/ebook/${book.id}`}
            className="w-60 cursor-pointer rounded-2xl p-0 shadow-lg bg-white hover:shadow-2xl transition overflow-hidden"
          >
            <Image
              src={book.cover}
              alt={book.title}
            ...
            <div className="py-2 px-4 flex flex-col gap-1">
              ...
              <button className="bg-green-900 text-white px-6 py-2 rounded-full text-lg hover:bg-green-800 transition mt-2 w-full">
                I Need This
              </button>
            </div>
          </Link>
